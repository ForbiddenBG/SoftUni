...
class Laptop(Computer):
    ...
    def configure_computer(self, processor, ram):
        if processor not in available_components.laptop_available_processors:
            raise ValueError(f"{processor} is not compatible with laptop "
                             f"{self.manufacturer} {self.model}!")
        if ram not in available_components.laptop_available_rams:
            raise ValueError(f"{ram}GB RAM is not compatible with laptop "
                             f"{self.manufacturer} {self.model}!")
        self.processor = processor
        self.ram = ram
        total_price = available_components.laptop_available_processors[processor] + \
                      available_components.laptop_available_rams[ram]
        self.price = total_price
        return f"Created {self.manufacturer} {self.model} with " \
               f"{self.processor} and {ram}GB RAM for {self.price}$."
